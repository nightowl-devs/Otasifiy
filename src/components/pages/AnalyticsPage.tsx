...
function fillMonthlySeries(monthly: { month: string; downloads: number }[]): { month: string; downloads: number }[] {
...
export function AnalyticsTimeRangePicker({ currentRange, onChange }: AnalyticsTimeRangePickerProps) {
  const activeIndex = RANGES.findIndex((range) => range.value === currentRange);
  const offset = activeIndex === RANGES.length - 1 ? "- 0.250rem" : activeIndex === 0 ? "+ 0.250rem" : "";
  ...
  return (
    <div className="rounded-lg border border-zinc-700 bg-zinc-800/80 p-4">
      ...
      <div className="flex items-center gap-1 mt-1">
        ...
        <span className={`text-xs ${positive ? "text-emerald-400" : "text-red-400"}`}>{delta}</span>
      </div>
    </div>
  );
}
...
export default function AnalyticsPage() {
  ...
  const { data, isLoading, isError, refetch } = useAnalytics(project.id, timeRange);

  const monthly = useMemo(() => (data ? fillMonthlySeries(data.monthly) : []), [data]);
  const daily = useMemo(() => (data ? fillDailySeries(data.daily, timeRange) : []), [data, timeRange]);
  ...
  const [hasZeroMotion, setHasZeroMotion] = useState(false);
  useMemo(() => {
    if (data?.totals.totalDownloads === 0) {
      setHasZeroMotion(true);
    }
  }, [data]);

  if (isLoading && !hasZeroMotion) {
    return (
  ...
  return (
    <div className="mx-auto max-w-6xl p-8 overflow-hidden">
      <div className="flex items-center justify-between mb-6">
        ...
        <AnalyticsTimeRangePicker currentRange={timeRange} onChange={(range) => setTimeRange(range)} />
      </div>

      {hasZeroMotion ? (
        <div className="flex flex-col items-center justify-center gap-3 rounded-lg  p-8 border border-zinc-700 bg-zinc-800/80 ">
          <p className="text-zinc-300  text-2xl font-semibold">your bitchass app has ZERO USERS </p>
          <p className="text-zinc-400 text-sm">
            Your app's updates haven't been downloaded yet. Once users start downloading your updates, you'll see the analytics
            here.
          </p>
        </div>
      ) : (
        <>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="rounded-lg border border-zinc-700 bg-zinc-800/80 p-4">
              <h2 className="text-base font-semibold text-zinc-100 mb-4">Monthly Downloads</h2>
              <ChartContainer config={monthlyConfig} className="h-64 w-full">
                <BarChart data={monthly}>
                  ...
                  />
                  <YAxis tickLine={false} axisLine={false} tickMargin={8} stroke="#a1a1aa" />
            ...
            <div className="rounded-lg border border-zinc-700 bg-zinc-800/80 p-4">
              <h2 className="text-base font-semibold text-zinc-100 mb-4">Daily Downloads</h2>
            ...
            <div className="rounded-lg border border-zinc-700 bg-zinc-800/80 p-4">
              <h2 className="text-base font-semibold text-zinc-100 mb-4">Platform Distribution</h2>
              <ChartContainer config={platformConfig} className="h-64 w-full">
                <PieChart>
                  <ChartTooltip content={<ChartTooltipContent />} />
                  <Pie data={platformData} dataKey="users" nameKey="platform" innerRadius={60} outerRadius={100} paddingAngle={2}>
                    <LabelList dataKey="platform" className="fill-zinc-300 font-medium" />
                  </Pie>
                </PieChart>
              </ChartContainer>
            </div>
            ...
            <div className="rounded-lg border border-zinc-700 bg-zinc-800/80 p-4">
              <h2 className="text-base font-semibold text-zinc-100 mb-4">Runtime Versions</h2>
              <ChartContainer config={runtimeConfig} className="h-64 w-full">
                <BarChart data={runtimeData} layout="vertical">
                  ...
                  <XAxis type="number" tickLine={false} tickMargin={8} axisLine={false} stroke="#a1a1aa" unit="%" />
                  <YAxis dataKey="version" type="category" tickLine={false} tickMargin={8} axisLine={false} stroke="#a1a1aa" />
                  <ChartTooltip content={<ChartTooltipContent hideIndicator />} />
                  <Bar dataKey="pct" fill="var(--color-pct)" radius={[0, 4, 4, 0]}>
                    <LabelList dataKey="pct" position="right" className="fill-zinc-300 font-medium" formatter={(v) => `${v}%`} />
                  </Bar>
                </BarChart>
              </ChartContainer>
            </div>
